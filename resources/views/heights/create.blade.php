...
@section('content')
    <div class="card push-top w-50 m-auto mt-2">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('heights.store') }}">
                @csrf
                <div class="form-group mb-2">
                    <label for="height_range">Рост</label>
                    <input type="text" class="form-control @error('height_range') is-invalid @enderror" name="height_range" id="height_range" value="{{old('height_range')}}"/>
                    @error('height_range')
