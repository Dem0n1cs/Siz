...
@section('content')
    <div class="card push-top w-50 m-auto mt-2">
        ...
        <div class="card-body">
            <form method="post" action="{{ route('clothing_sizes.store') }}">
                @csrf
                <div class="form-group mb-2">
                    <label for="size_range">Размер</label>
                    <input type="text" class="form-control @error('size_range') is-invalid @enderror" name="size_range" id="size_range" value="{{old('size_range')}}"/>
                    @error('size_range')
