        ...
        <div class="card-body">
            <form method="post" action="{{ route('equipment.update',$equipment->id) }}">
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" name="title" id="title" value="{{old('title',$equipment->title)}}"/>
                    @error('title')
                ...
                <div class="form-group mb-2">
                    ...
                    <select class="form-control @error('classification_id') is-invalid @enderror" name="classification_id" id="classification_id">
                        ...
                        @foreach($classifications as $key=>$value)
                            <option value="{{$key}}" @selected(old('classification_id',$equipment->classification_id) == $key)>{{$value}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="text" class="form-control @error('wear_period') is-invalid @enderror" name="wear_period" id="wear_period" value="{{old('wear_period',$equipment->wear_period)}}"/>
                    @error('wear_period')
                    <span class="invalid-feedback fs-6">{{ $message }}</span>
                    @enderror
                </div>
