            ...
            <form method="post" action="{{ route('users.update', $user->id) }}">
                ...
                <div class="form-group mb-2">
                    ...
                    <select class="form-control @error('division_id') is-invalid @enderror"
                        ...
                        @foreach($divisions as $key=>$full_title)
                            <option value="{{$key}}" @selected(old('division_id',$user->division_id) == $key)>{{$full_title}}</option>
                        @endforeach
                    </select>
                    @error('division_id')
                    <span class="invalid-feedback fs-6">{{ $message }}</span>
                ...
                <div class="form-group mb-2">
                    ...
                    <select class="form-control @error('profession_id') is-invalid @enderror"
                        ...
                        @foreach($professions as $key=>$title)
                            <option value="{{$key}}" @selected(old('profession_id',$user->profession_id) == $key)>{{$title}}</option>
                        @endforeach
                    </select>
                    @error('profession_id')
                    <span class="invalid-feedback fs-6">{{ $message }}</span>
                ...
                <div class="form-group mb-2">
                    ...
                    <input type="date"
                           ...
                           name="employment"
                           id="employment"
                           value="{{old('employment',$user->employment)}}"/>
                    @error('employment')
                    <span class="invalid-feedback fs-6">{{ $message }}</span>
                    @enderror
                </div>
